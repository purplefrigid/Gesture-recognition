...
u16 USART1_RX_STA=0; 
__align(8) u8 USART1_TX_BUF[USART2_MAX_SEND_LEN];
u8 USART1_RX_BUF[USART2_MAX_RECV_LEN];
//////////////////////////////////////////////////////////////////////////////////	 
...
void uart_init(u32 bound){
  ...
  USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);//开启串口接受中断
	UART_DMA_Config(DMA1_Channel7,(u32)&USART1->DR,(u32)USART1_TX_BUF);
	...
	void USART1_IRQHandler(void)
{
	...
	{	 
		...
		Instruction=USART_ReceiveData(USART1);
		LCD_Clear(WHITE);
		if(USART1_RX_STA<USART2_MAX_RECV_LEN)		//还可以接收数据
		{
			TIM_SetCounter(TIM4,0);//计数器清空        				 
			if(USART1_RX_STA==0)TIM4_Set(1);	 	//使能定时器4的中断 
			USART1_RX_BUF[USART2_RX_STA++]=res;		//记录接收到的值	 
		}else 
		{
